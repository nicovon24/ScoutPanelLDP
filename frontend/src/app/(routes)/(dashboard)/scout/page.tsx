...
export default function ScoutPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-100px)] overflow-hidden bg-[#0F0F0F] -m-6 rounded-none text-white font-sans antialiased">

      {/* ── Sidebar Filters ── */}
      <aside className="w-[360px] border-r border-white-[0.05] bg-[#121212] overflow-y-auto p-6 space-y-7 custom-scrollbar">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-[17px] font-bold">Filtros</h2>
            {hasFilters && (
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center gap-2 bg-[#1E1E1E] border border-white/10 px-3 py-1.5 rounded-sm text-[10px] font-bold text-white/70">
              EDAD: {ageMin} - {ageMax}
            </div>
            {activePos && (
              <div className="flex items-center gap-2 bg-[#1E1E1E] border border-white/10 px-3 py-1.5 rounded-sm text-[10px] font-bold text-white/70">
                POS: {activePos} <X size={12} className="text-muted cursor-pointer hover:text-white" onClick={() => setActivePos("")} />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4 pt-2">
          <p className="text-[13px] font-bold uppercase tracking-tight">Edad</p>
          <div className="flex justify-between items-center gap-4">
            <div className="flex flex-col gap-1 w-full">
              <label className="text-[10px] text-muted font-bold">MÍNIMA</label>
              <input type="number" min="15" max="45" value={ageMin} onChange={(e) => setAgeMin(e.target.value)}
                className="bg-[#1A1A1A] border border-white/10 rounded-md px-3 py-2 text-center text-[13px] font-bold text-white outline-none focus:border-green transition-colors w-full" />
            </div>
            <div className="flex flex-col gap-1 w-full">
              <label className="text-[10px] text-muted font-bold">MÁXIMA</label>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-1.5">
            <p className="text-[13px] font-bold uppercase tracking-tight">Posición</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {["PORTERO", "DEFENSA", "MEDIOCAMPO", "DELANTERO"].map(p => (
              <button
                ...
                className={`text-[10px] py-3 rounded border font-black tracking-tight transition-all
                            ${activePos === p
                    ? "bg-[#182622] border-green text-green"
                    : "bg-[#1A1A1A] border-white/5 text-[#555555] hover:text-white/60"}`}
              >
                {p}
              </button>
            ))}
        ...
        <header className="h-20 flex items-center px-10 gap-10 bg-[#121212]">
          ...
          <div className="flex-1 max-w-2xl relative">
            <div className="flex items-center bg-[#1A1A1A] rounded-full border border-white/[0.08] h-11 px-6 group transition-all focus-within:border-white/20">
              <div className="flex items-center gap-2 pr-4 border-r border-white/5 text-[13px] font-bold text-green mr-4 cursor-pointer hover:text-green-dark">
                Jugadores
              </div>
              <input value={inputQ} onChange={(e) => setInputQ(e.target.value)} placeholder="Buscar por apellido..." className="bg-transparent border-none outline-none text-sm text-white w-full placeholder:text-[#444444]" />
              {inputQ ? (
                <X size={18} className="text-muted cursor-pointer hover:text-white" onClick={() => setInputQ("")} />
              ) : (
                <Search size={18} className="text-[#444444] cursor-pointer group-focus-within:text-white transition-colors" />
              )}
            </div>
          </div>
        </header>
          ...
          {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 bg-[#0A0A0A] z-10 border-b border-white/[0.04] shadow-sm">
                <tr className="text-[10px] text-white/40 uppercase font-bold tracking-[2px]">
                  ...
                  <tr key={p.id} className="group hover:bg-white-[0.015] transition-all animate-fade-in duration-300">
                    ...
                    <td className="px-4 py-[18px]">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-0.5">
                          <p className="text-[13.5px] font-bold text-white/90 group-hover:text-green transition-colors">{p.name}</p>
                          <p className="text-[11px] text-white/30 font-medium">{p.nationality}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-[18px] text-[13px] font-bold text-white/60">
                      {p.age}
                    </td>
                    <td className="px-4 py-[18px]">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full border border-white/5 flex items-center justify-center relative bg-[#121212]">
                          ...
                          <span className="text-[12px] font-bold text-white">{p.matchPct}</span>
                        </div>
                    ...
                    <td className="px-4 py-[18px]">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[13px] text-white/70 truncate max-w-[160px] font-medium">{p.team?.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-[18px]">
                      <div className="flex items-center gap-3">
                        <div className="w-1.5 h-1.5 rounded-sm bg-green opacity-70" />
                        <span className="text-[12.5px] text-white/50 uppercase font-black tracking-tight">{p.position}</span>
                      </div>
                    </td>
                    <td className="px-4 py-[18px] text-[13px] font-bold text-white/30 truncate">
                      {p.contractUntil || "12/27"}
                    </td>
                    <td className="px-4 py-[18px] text-[13px] font-bold text-white/30">
                      {p.height || "1.8 mts"}
                    </td>
                    <td className="px-4 py-[18px] text-[13px] font-bold text-white/30 uppercase">
                      {p.foot || "Der"}
                    </td>
                  </tr>
                ))}
