  ...
  return (
    <Link href="/" className="flex items-center gap-3 group">
      ...
      <div className="flex flex-col">
        <span className="text-[17px] font-black text-primary leading-tight tracking-[0.02em] uppercase">
          librodepases
        </span>
        <span className="text-[9px] text-green font-black uppercase tracking-[0.2em] -mt-0.5">Scouting Panel</span>
      </div>
    </Link>
  );
}
...
export default function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-base/80 backdrop-blur-xl border-b border-white/[0.04]
      ...
      <div className="w-1/4 flex items-center justify-end gap-5 flex-shrink-0">
        ...
        <div className="flex items-center gap-3 pl-3">
          <div className="flex flex-col items-end">
            <span className="text-[13px] font-bold text-primary">{user?.name ?? "Scout"}</span>
            <span className="text-[10px] text-muted uppercase font-black tracking-widest">PRO Account</span>
          </div>
